...
// "Administration > Freight Masters > Container Service Mode / [CODE] Name"
// detail view, mirroring CargoFlo ERP: breadcrumb with record pagination
// (1/5, with prev/next chevrons), an "Action" dropdown (Archive +
// Confirmation popup), Name, Code, Active toggle, and a Summary notes tab.
const ContainerServiceModeDetail = () => {
  ...
  const index = CONTAINER_SERVICE_MODES.findIndex((csm) => csm.code === decodedId);
  const item = CONTAINER_SERVICE_MODES[index] || CONTAINER_SERVICE_MODES[0];
  const total = CONTAINER_SERVICE_MODES.length;
  ...
  const goRelative = (delta) => {
    ...
    navigate(`/admin/administration/freight-masters/container-service-modes/${encodeURIComponent(CONTAINER_SERVICE_MODES[nextIndex].code)}`);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm">
          <button
            onClick={() => navigate('/admin/administration/freight-masters/container-service-modes')}
...
export default ContainerServiceModeDetail;
