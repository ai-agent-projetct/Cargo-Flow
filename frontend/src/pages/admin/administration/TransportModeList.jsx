...
const TransportModeList = () => {
  ...
  const filtered = TRANSPORT_MODES.filter((tm) =>
    !search ||
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-xl font-bold text-slate-900">Transport Mode</h2>
      </div>

      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <MasterListToolbar
          rows={filtered}
          filename="transport-mode"
          search={search}
          onSearch={setSearch}
        />
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
              <tr>
                ...
                <th className="text-left px-4 py-3">Mode Type</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((tm) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-primary-600">{tm.code}</td>
                  <td className="px-4 py-3 text-primary-600">{tm.name}</td>
                  <td className="px-4 py-3 text-primary-600">{tm.modeType}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center justify-center w-9 h-5 rounded-full ${tm.active ? 'bg-primary-600' : 'bg-slate-300'}`}>
                      <span className="text-[10px] text-white">{tm.active ? '✓' : ''}</span>
                    </span>
                  </td>
                </tr>
              ))}
