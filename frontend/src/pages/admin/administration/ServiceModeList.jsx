...
const ServiceModeList = () => {
  ...
  const filtered = SERVICE_MODES.filter((sm) =>
    !search ||
    sm.name.toLowerCase().includes(search.toLowerCase()) ||
    sm.code.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-xl font-bold text-slate-900">Service Mode</h2>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((sm) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-primary-600">{sm.code}</td>
                  <td className="px-4 py-3 text-primary-600">{sm.name}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center justify-center w-9 h-5 rounded-full ${sm.active ? 'bg-primary-600' : 'bg-slate-300'}`}>
                      <span className="text-[10px] text-white">{sm.active ? '✓' : ''}</span>
                    </span>
                  </td>
                </tr>
              ))}
