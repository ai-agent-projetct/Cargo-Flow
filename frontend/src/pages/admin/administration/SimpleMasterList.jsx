...
const SimpleMasterList = () => {
  ...
  // Export whatever this reference list holds.
  const handleExport = () => {
    if (exportCsv(config.rows || [], null, slug || 'freight-master')) {
      toast.success(`Exported ${(config.rows || []).length} rows`);
    } else {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            {/* Freight Masters are reference lists the system ships with, so
                there is nothing here for a user to add. */}
            <button disabled title="This is a system reference list and cannot be added to"
              className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium opacity-40 cursor-not-allowed">
