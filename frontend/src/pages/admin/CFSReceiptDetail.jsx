...
import {
  Save, X, ChevronDown, ChevronLeft, ChevronRight, MoreHorizontal, Printer,
  Trash2, FileText,
} from 'lucide-react';
...
const STATUS_LABELS = { created: 'Created', received: 'Received', stuffed: 'Stuffed', cancelled: 'Cancelled' };
...
const CFSReceiptDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchRecord = async () => {
      setLoading(true);
      try {
        const res = await cfsReceiptsAPI.getById(id);
        const data = res.data?.data;
        setForm((f) => ({ ...emptyForm, ...data }));
      } catch {
        toast.error('Failed to load CFS receipt');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    cfsReceiptsAPI.getAll({ limit: 1000, sortBy: 'createdAt', sortOrder: 'DESC' })
  ...
  const goToRecord = (direction) => {
    ...
    navigate(`/admin/cfs-receipts/${recordList[newIndex]}`);
  };
  ...
  const handleStatusChange = async (status) => {
    ...
    try {
      await cfsReceiptsAPI.updateStatus(id, status);
      setField('status', status);
  ...
  const openStatusModal = () => {
    if (isNew) {
      toast('Save the receipt first', { icon: 'ℹ️' });
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm('Delete this CFS receipt? This cannot be undone.')) return;
    try {
      await cfsReceiptsAPI.delete(id);
      toast.success('CFS Receipt deleted');
      navigate('/admin/cfs-receipts');
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        const res = await cfsReceiptsAPI.create(payload);
        const created = res.data?.data;
        toast.success('CFS Receipt created');
        navigate(`/admin/cfs-receipts/${created.id}`);
      } else {
        await cfsReceiptsAPI.update(id, payload);
        toast.success('CFS Receipt updated');
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Failed to save CFS receipt');
    } finally {
      setSaving(false);
    }
  };

  const handleDiscard = () => navigate('/admin/cfs-receipts');
    ...
    <div className="p-6 space-y-4 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm text-gray-500">
          CFS / Receive Entry / {isNew ? 'New' : (form.receiptNumber || 'New')}
        </div>
        {!isNew && recordIndex !== -1 && (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-2xl font-bold text-gray-900 font-mono">{isNew ? 'New' : (form.receiptNumber || 'New')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="date" className={inputClass} value={form.gateInDate ? String(form.gateInDate).slice(0, 10) : ''} onChange={(e) => setField('gateInDate', e.target.value)} />
            </div>
            <div>
      ...
      {/* Gate-In / Vehicle Details */}
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
        <h2 className="font-bold text-gray-900">Gate-In Details</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          <div className="space-y-4">
            <div>
      ...
      {!isNew && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-3">
          ...
          <div className="space-y-2 text-sm text-gray-600">
            <div className="flex items-center justify-between">
              <span>Receive Entry created</span>
...
export default CFSReceiptDetail;
